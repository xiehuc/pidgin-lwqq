...
static void set_user_list(qq_chat_group* cg)
{
   ...
   if (purple_conv_chat_get_users(PURPLE_CONV_CHAT(conv)) == NULL) {
      LIST_FOREACH(member, &group->members, entries)
      {
         ...
         if (lwqq_member_is_founder(member, group))
            flag |= PURPLE_CBFLAGS_FOUNDER;
         if (member->stat != LWQQ_STATUS_OFFLINE)
            flag |= PURPLE_CBFLAGS_VOICE;
         if (member->mflag & LWQQ_MEMBER_IS_ADMIN)
         ...
         if ((buddy = find_buddy_by_uin(lc, member->uin))) {
            if (ac->flag & QQ_USE_QQNUM)
               users
                   = g_list_append(users, try_get(buddy->qqnumber, buddy->uin));
            else
...
void qq_cgroup_got_msg(qq_chat_group* cg, const char* serv_id,
                       PurpleMessageFlags flags, const char* message, time_t t)
{
   ...
   if (b == NULL)
      sb = lwqq_group_find_group_member_by_uin(cg->group, serv_id);
   if (cg->group->mask > 0 && CGROUP_GET_CONV(cg) == NULL) {
      if (cg_->unread_num == 0) {
         cg_->log = purple_log_new(PURPLE_LOG_CHAT, cg->group->account,
                                   cg->chat->account, NULL, t, NULL);
      }
      name = b ? (b->markname ?: b->nick)
               : (sb ? (sb->card ?: sb->nick) : serv_id);
      ...
      set_user_list(cg);
      name = b ? (b->qqnumber ?: b->nick)
               : (sb ? (sb->card ?: sb->nick) : serv_id);
      PurpleConversation* conv = CGROUP_GET_CONV(cg);
      serv_got_chat_in(gc, purple_conv_chat_get_id(PURPLE_CONV_CHAT(conv)),
                       name, flags, message, t);
   }
}
