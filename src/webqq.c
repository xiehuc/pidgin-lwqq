...
static LwqqSimpleBuddy* find_group_member_by_nick_or_card(LwqqGroup* group,
                                                          const char* who)
{
   if (!group || !who)
      return NULL;
 ...
 */
static int find_group_and_member_by_card(LwqqClient* lc, const char* card,
                                         LwqqGroup** p_g,
                                         LwqqSimpleBuddy** p_sb)
{
   if (!card)
      return 0;
   char nick[128] = { 0 };
...
static void visit_self_infocenter(PurplePluginAction* action)
{
   ...
   snprintf(url, sizeof(url),
            OPEN_PROG " 'http://user.qzone.qq.com/%s/infocenter'",
            ac->qq->myself->uin);
   system(url);
}
static void user_help(PurplePluginAction* action)
{
   ...
   OPEN_URL(cmd,
            _("https://github.com/xiehuc/pidgin-lwqq/wiki/simple-user-guide"));
   system(cmd);
}
...
void qq_all_reset(qq_account* ac, QQResetFlags opt)
{
   ...
   if (opt
       & (RESET_GROUP | RESET_DISCU | RESET_GROUP_SOFT | RESET_DISCU_SOFT)) {
      // PurpleGroup* group = purple_find_group("QQ群");
      PurpleBlistNode* group, *node;
      for (group = purple_get_blist()->root; group != NULL;
           group = group->next) {
         node = group->child;
         while (node != NULL) {
            if (PURPLE_BLIST_NODE_IS_CHAT(node)) {
               ...
               if (purple_chat_get_account(chat) == ac->account) {
                  ...
                  if (type == NULL)
                     ...
                     if ((strcmp(type, QQ_ROOM_TYPE_DISCU) == 0
                          && opt & RESET_DISCU_SOFT)
                         || (strcmp(type, QQ_ROOM_TYPE_QUN) == 0
                             && opt & RESET_GROUP_SOFT)) {
                        const char* name = get_name_from_chat(chat);
                        if (lwqq_group_find_group_by_qqnumber(ac->qq, name)
                            == NULL)
                           purple_blist_remove_chat(chat);
                     } else
                        purple_blist_remove_chat(chat);
...
static void search_group_receipt(LwqqAsyncEvent* ev, LwqqGroup* g)
{
   ...
   if (err == 10000) {
      ...
      lwqq_async_add_event_listener(event,
                                    _C_(2p, search_group_receipt, ev, g));
      return;
   }
   if (err == LWQQ_EC_NO_RESULT) {
      purple_notify_message(ac->gc, PURPLE_NOTIFY_MSG_INFO, _("Error Message"),
                            _("Get QQ Group Infomation Failed"), NULL, NULL,
                            NULL);
      lwqq_group_free(g);
      return;
   }
...
static void qq_add_group(PurplePluginAction* action)
{
   ...
   purple_request_input(gc, _("Add QQ Group"), _("QQ Number"), NULL, NULL,
                        FALSE, FALSE, NULL, _("Search"),
                        G_CALLBACK(search_group), _("Cancel"),
                        G_CALLBACK(do_no_thing), ac->account, NULL, NULL, ac);
}
static void create_discu(qq_account* ac, PurpleRequestFields* root)
{
   ...
   while ((piece = strsep(&ptr, ";"))) {
      ...
      if (b == NULL)
         b = lwqq_buddy_find_buddy_by_name(lc, piece);
      if (b)
   ...
   if (err[0])
      purple_notify_warning(ac->account, _("Warning"),
                            _("Couldn't find friend"), err);
...
static void qq_create_discu(PurplePluginAction* action)
{
   ...
   leaf = purple_request_field_string_new("name", _("Discussion Name"),
                                          _("UnName Discussion"), FALSE);
   purple_request_field_string_set_editable(leaf, TRUE);
   purple_request_field_group_add_field(branch, leaf);
   leaf = purple_request_field_string_new(
       "members", _("members(seperate by ;)"), "", TRUE);
   purple_request_field_string_set_editable(leaf, TRUE);
   purple_request_field_group_add_field(branch, leaf);

   purple_request_fields(gc, _("Create Discussion"), NULL, NULL, root,
                         _("Create"), G_CALLBACK(create_discu), _("Cancel"),
                         G_CALLBACK(do_no_thing), ac->account, NULL, NULL, ac);
}
...
static GList* plugin_actions_menu(PurplePlugin* UNUSED(plugin),
                                  gpointer context)
{
   ...
   m = g_list_append(m, act);
   act = purple_plugin_action_new(_("Create Discussion Group"),
                                  qq_create_discu);
   m = g_list_append(m, act);
   act = purple_plugin_action_new(_("Modify Self LongNick"),
                                  qq_modify_self_longnick);
   m = g_list_append(m, act);
...
static const char* qq_list_icon(PurpleAccount* UNUSED(a),
                                PurpleBuddy* UNUSED(b))
{
   return "webqq";
}
...
#define WEBQQ_STATUS_TYPE_ATTR                                                 \
   TRUE, TRUE, FALSE, "nick", "nick", purple_value_new(PURPLE_TYPE_STRING), NULL

   // adium require available status
   status = purple_status_type_new_with_attrs(PURPLE_STATUS_AVAILABLE,
                                              "available", _("Available"),
                                              WEBQQ_STATUS_TYPE_ATTR);
   types = g_list_append(types, status);
   status = purple_status_type_new_with_attrs(
       PURPLE_STATUS_AVAILABLE, "online", _("Online"), WEBQQ_STATUS_TYPE_ATTR);
   types = g_list_append(types, status);
   status = purple_status_type_new_with_attrs(
       PURPLE_STATUS_AVAILABLE, "callme", _("Callme"), WEBQQ_STATUS_TYPE_ATTR);
   types = g_list_append(types, status);
   status = purple_status_type_new_with_attrs(
       PURPLE_STATUS_AWAY, "away", _("Away"), WEBQQ_STATUS_TYPE_ATTR);
   types = g_list_append(types, status);
   status = purple_status_type_new_with_attrs(
       PURPLE_STATUS_UNAVAILABLE, "busy", _("Busy"), WEBQQ_STATUS_TYPE_ATTR);
   types = g_list_append(types, status);
   ...
   types = g_list_append(types, status);
   status = purple_status_type_new_with_attrs(
       PURPLE_STATUS_INVISIBLE, "hidden", _("Hidden"), WEBQQ_STATUS_TYPE_ATTR);
   types = g_list_append(types, status);
   status = purple_status_type_new_with_attrs(
       PURPLE_STATUS_OFFLINE, "offline", _("Offline"), WEBQQ_STATUS_TYPE_ATTR);
   types = g_list_append(types, status);
   status = purple_status_type_new_with_attrs(
       PURPLE_STATUS_MOBILE, "mobile", _("Online"), WEBQQ_STATUS_TYPE_ATTR);
   types = g_list_append(types, status);
...
static void qq_set_status(PurpleAccount* account, PurpleStatus* status)
{
   qq_account* ac = purple_connection_get_protocol_data(
       purple_account_get_connection(account));
   lwqq_info_change_status(ac->qq,
                           qq_status_from_str(purple_status_get_id(status)));
}

#define buddy_status(bu)                                                       \
   ((bu->stat == LWQQ_STATUS_ONLINE && bu->client_type == LWQQ_CLIENT_MOBILE)  \
        ? "mobile"                                                             \
...
void friend_come(LwqqClient* lc, LwqqBuddy** p_buddy)
{
   ...
   if (bu == NULL) {
      bu = purple_buddy_new(ac->account, key,
                            (buddy->markname) ? buddy->markname : buddy->nick);
      purple_blist_add_buddy(bu, NULL, group, NULL);
      // if there isn't a qqnumber we shouldn't save it.
      if (!buddy->qqnumber)
         purple_blist_node_set_flags(PURPLE_BLIST_NODE(bu),
...
static void qq_set_group_name(qq_chat_group* cg)
{
   ...
   if (hide) {
      ...
      if (hide == 1) {
         ...
         unsigned int split = unread < 10 ? unread : unread < 100
                                                         ? unread / 10 * 10
                                                         : unread / 100 * 100;
         if (unread > 0)
            sprintf(gname + strlen(gname), "(%u%s)", split,
                    unread > 10 ? "+" : "");
      }
   }
...
void group_come(LwqqClient* lc, LwqqGroup** p_group)
{
   ...
   if ((chat = purple_blist_find_chat(account, key)) == NULL) {
      ...
      type = (group->type == LWQQ_GROUP_QUN) ? QQ_ROOM_TYPE_QUN
                                             : QQ_ROOM_TYPE_DISCU;
      ...
      if (!group->account)
         purple_blist_node_set_flags(PURPLE_BLIST_NODE(chat),
                                     PURPLE_BLIST_NODE_FLAG_NO_SAVE);
   } else {
      ...
      if (!g_hash_table_lookup(components, QQ_ROOM_TYPE)) {
         type = (group->type == LWQQ_GROUP_QUN) ? QQ_ROOM_TYPE_QUN
                                                : QQ_ROOM_TYPE_DISCU;
         g_hash_table_insert(components, s_strdup(QQ_ROOM_TYPE),
                             g_strdup(type));
      }
   ...
   if (lwqq_group_is_qun(group)) {
      ...
      if (purple_buddy_icons_node_has_custom_icon(PURPLE_BLIST_NODE(chat))
          == 0) {
...
static void buddy_message(LwqqClient* lc, LwqqMsgMessage* msg)
{
   ...
   const char* local_id = (ac->flag & QQ_USE_QQNUM) ? buddy->qqnumber
                                                    : buddy->uin;
...
static void offline_file(LwqqClient* lc, LwqqMsgOffFile* msg)
{
   ...
   serv_got_im(pc, serv_id_to_local(ac, msg->super.from), buf,
...
static void notify_offfile(LwqqClient* lc, LwqqMsgNotifyOfffile* notify)
{
   ...
   PurpleConnection* pc = ac->gc;
   char buf[512] = { 0 };
   const char* action = (notify->action == NOTIFY_OFFFILE_REFUSE) ? _("Refuse")
                                                                  : _("Agree");
   snprintf(buf, sizeof(buf), _("Peer %s Accept receive Offline file(%s)\n"),
            action, notify->filename);
   serv_got_im(pc, serv_id_to_local(ac, notify->super.from), buf,
...
static void request_join_confirm(LwqqClient* lc, LwqqMsgSysGMsg* msg,
                                 LwqqConfirmTable* ct)
{
...
static void sys_g_request_join(LwqqClient* lc, LwqqBuddy* buddy,
                               LwqqMsgSysGMsg* msg)
{
   char body[1024] = { 0 };
...
static void sys_g_message(LwqqClient* lc, LwqqMsgSysGMsg* msg)
{
   ...
   case GROUP_LEAVE: {
      ...
      PurpleChat* chat = purple_blist_find_chat(
          ac->account, try_get(msg->group->account, msg->group->gid));
      if (chat && msg->is_myself) {
         purple_blist_remove_chat(chat);
      }
   } break;
   case GROUP_REQUEST_JOIN_DENY:
      snprintf(body, sizeof(body), _("QQ Group [%s] refused your request"),
               msg->account);
      break;
   case GROUP_REQUEST_JOIN: {
      ...
      lwqq_async_add_event_listener(
          ev, _C_(3p, sys_g_request_join, lc, buddy, nmsg));
      return;
   } break;
   default:
      return;
   ...
   purple_notify_message(ac->gc, PURPLE_NOTIFY_MSG_INFO,
                         _("QQ Group Sys Message"), body, NULL, NULL, NULL);
   qq_system_log(ac, body);
}
...
static void rewrite_whole_message_list(LwqqAsyncEvent* ev, qq_account* ac,
                                       LwqqGroup* group)
{
   ...
   if (conv == NULL) {
      ...
      while (item) {
         ...
         if (entry->owner == group) {
            ...
            ac->rewrite_pic_list
                = g_list_remove_link(ac->rewrite_pic_list, safe);
         }
      }
      return;
   }
         ...
         while ((pic = strstr(pic, "<IMG")) != NULL) {
            ...
            while (item) {
               ...
               if (entry->ori_id == id) {
                  ...
                  s_free(entry);
                  ac->rewrite_pic_list
                      = g_list_remove_link(ac->rewrite_pic_list, item);
                  break;
               }
               item = item->next;
            }
   ...
   while (list) {
      message = list->data;
      qq_cgroup_got_msg(group->data, message->who, PURPLE_MESSAGE_RECV,
...
static int group_message(LwqqClient* lc, LwqqMsgMessage* msg)
{
   ...
   if (msg->super.super.type == LWQQ_MS_GROUP_WEB_MSG) {
      ...
      case 1:
         snprintf(lost_msg, sizeof(lost_msg), "lost message from #%d to #%d",
                  seq + 1, msg->group.seq - 1);
         qq_cgroup_got_msg(group->data, msg->group.send, PURPLE_MESSAGE_ERROR,
                           lost_msg, time(0));
         break;
      case -1:
...
static void whisper_message(LwqqClient* lc, LwqqMsgMessage* mmsg)
{
   ...
   LwqqCommand cmd = _C_(4pl, whisper_message_delay_display, ac, group,
                         s_strdup(from), s_strdup(ds_c_str(buf)), mmsg->time);
   ds_free(buf);
   if (LIST_EMPTY(&group->members)) {
      lwqq_async_add_event_listener(
          lwqq_info_get_group_detail_info(lc, group, NULL), cmd);
   } else
      vp_do(cmd, NULL);
}
static void whisper_message_delay_display(qq_account* ac, LwqqGroup* group,
                                          char* from, char* msg, time_t t)
{
   ...
   if (sb == NULL) {
      ...
      snprintf(name, sizeof(name), "%s ### %s",
               (sb->card) ? sb->card : sb->nick, group->name);
   }
...
static void status_change(LwqqClient* lc, LwqqMsgStatusChange* status)
{
   ...
   if (ac->flag & QQ_USE_QQNUM) {
      ...
      if (buddy == NULL || buddy->qqnumber == NULL)
         return;
      who = buddy->qqnumber;
   } else {
      who = status->who;
   ...
   purple_prpl_got_user_status(
       account, who,
       status->client_type == LWQQ_CLIENT_MOBILE ? "mobile" : status->status,
       NULL);
}
static void kick_message(LwqqClient* lc, LwqqMsgKickMessage* kick)
{
   ...
   if (kick->show_reason)
      ...
      reason = _("Oups, You was kicked offline by somewhat");
   purple_connection_error_reason(ac->gc, PURPLE_CONNECTION_ERROR_OTHER_ERROR,
                                  reason);
}
static void verify_required_confirm(LwqqClient* lc, char* account,
                                    LwqqConfirmTable* ct)
{
                           ...
                           LwqqBuddy* buddy)
{
   ...
   if (system->type == VERIFY_REQUIRED) {
      ...
      ct->flags = LWQQ_CT_ENABLE_IGNORE;
      ct->cmd
          = _C_(3p, verify_required_confirm, lc, s_strdup(system->account), ct);
      show_confirm_table(lc, ct);
      lwqq_buddy_free(buddy);
      ...
      snprintf(buf1, sizeof(buf1),
               _("%s accept your request,and add back you as friend too"),
               system->account);
      purple_notify_message(ac->gc, PURPLE_NOTIFY_MSG_INFO, _("System Message"),
                            _("Add Friend"), buf1, NULL, NULL);

      qq_system_log(ac, buf1);
   } else if (system->type == VERIFY_PASS) {
      snprintf(buf1, sizeof(buf1), _("%s accept your request"),
               system->account);
      purple_notify_message(ac->gc, PURPLE_NOTIFY_MSG_INFO, _("System Message"),
                            _("Add Friend"), buf1, NULL, NULL);
...
static void blist_change(LwqqClient* lc, LwqqMsgBlistChange* blist)
{
   ...
   LIST_FOREACH(sb, &blist->added_friends, entries)
   {
      ...
      if (!buddy->qqnumber) {
         ...
         lwqq_async_add_evset_listener(set,
                                       _C_(2p, write_buddy_to_db, lc, buddy));
   ...
   LIST_FOREACH(buddy, &blist->removed_friends, entries)
   {
      ...
      PurpleConversation* conv = purple_find_conversation_with_account(
          PURPLE_CONV_TYPE_IM, key, account);
      if (conv)
...
void friend_avatar(qq_account* ac, LwqqBuddy* buddy)
{
   ...
   if (strcmp(buddy->uin, purple_account_get_username(account)) == 0) {
      purple_buddy_icons_set_account_icon(account, (guchar*)buddy->avatar,
                                          buddy->avatar_len);
   } else {
      purple_buddy_icons_set_for_user(account, key, (guchar*)buddy->avatar,
                                      buddy->avatar_len, NULL);
   }
   buddy->avatar = NULL;
   buddy->avatar_len = 0;
}
static void group_avatar(LwqqAsyncEvent* ev, LwqqGroup* group)
{
   ...
   purple_buddy_icons_node_set_custom_icon(
       PURPLE_BLIST_NODE(chat), (guchar*)group->avatar, group->avatar_len);
   // let free by purple;
   group->avatar = NULL;
   return;
}
                                ...
                                LwqqMsgContent** p_c, int* p_err)
{
   ...
   case LWQQ_CONTENT_OFFPIC:
      qq_sys_msg_write(lc->data, LWQQ_MS_BUDDY_MSG, serv_id,
                       _("Send Pic Failed"), PURPLE_MESSAGE_ERROR, time(NULL));
      break;
   case LWQQ_CONTENT_CFACE:
      if (find_group_by_gid(lc, serv_id)) {
         qq_sys_msg_write(lc->data, LWQQ_MS_GROUP_MSG, serv_id,
...
static void input_verify_image(LwqqVerifyCode* code,
...
static void cancel_verify_image(LwqqVerifyCode* code,
                                PurpleRequestField* fields)
{
   // valid client make sure it doesn't be freed
   LwqqClient* lc = code->lc;
   if (!lwqq_client_valid(lc))
   ...
   if (CAPTCHA_VIEW_OUTSIDE) {
      ...
      char cmd[512];
      snprintf(cmd, sizeof(cmd), OPEN_PROG " '%s/%s'", LWQQ_CACHE_DIR,
               lc->username);
      system(cmd);
...
static void show_verify_image(LwqqClient* lc, LwqqVerifyCode** p_code)
{
   ...
   code_pic = purple_request_field_image_new("code_pic", "code", code->data,
                                             code->size);
   purple_request_field_group_add_field(field_group, code_pic);

   code_entry
       = purple_request_field_string_new("code_entry", "input", "", FALSE);
   purple_request_field_set_required(code_entry, TRUE);
   purple_request_field_group_add_field(field_group, code_entry);

   purple_request_fields(
       ac->gc, NULL, _("Captcha"), NULL, fields, _("OK"),
       G_CALLBACK(input_verify_image),
       (CAPTCHA_VIEW_OUTSIDE) ? _("View Outside") : _("Cancel"),
       G_CALLBACK(cancel_verify_image), ac->account, NULL, NULL, code);

   return;
}

static void confirm_table_yes(LwqqConfirmTable* table,
                              PurpleRequestFields* fields)
{
...
static void confirm_table_no(LwqqConfirmTable* table,
                             PurpleRequestFields* fields)
{
   table->answer = (table->flags & LWQQ_CT_ENABLE_IGNORE) ? LWQQ_IGNORE
                                                          : LWQQ_NO;
   if (table->input_label) {
      const char* i = purple_request_fields_get_string(fields, "input");
      table->input = s_strdup(i);
...
static void show_confirm_table(LwqqClient* lc, LwqqConfirmTable* table)
{
   ...
   if (table->body) {
      PurpleRequestField* str = purple_request_field_string_new(
          "body", table->title, table->body, TRUE);
   ...
   if (table->exans_label || table->flags & LWQQ_CT_ENABLE_IGNORE
       || table->flags & LWQQ_CT_CHOICE_MODE) {
      PurpleRequestField* choice = purple_request_field_choice_new(
          "choice", _("Please Select"), table->answer);
   ...
   if (table->input_label) {
      PurpleRequestField* i = purple_request_field_string_new(
          "input", table->input_label, table->input ?: "", FALSE);
      s_free(table->input);
      purple_request_field_group_add_field(field_group, i);
   }

   purple_request_fields(
       ac->gc, NULL, _("Confirm"), NULL, fields, _("Confirm"),
       G_CALLBACK(confirm_table_yes),
       table->flags & LWQQ_CT_ENABLE_IGNORE ? _("Ignore") : _("Deny"),
       G_CALLBACK(confirm_table_no), ac->account, NULL, NULL, table);
}
static void delete_group_local(LwqqClient* lc, const LwqqGroup** p_g)
{
   ...
   PurpleConversation* conv = purple_find_conversation_with_account(
       PURPLE_CONV_TYPE_CHAT, key, ac->account);
   if (conv)
      purple_conversation_destroy(conv);
...
static void msg_unsend_print_reason(qq_account* ac, LwqqMsg* msg,
                                    const char* serv_id)
{
   qq_sys_msg_write(ac, msg->type, serv_id, _("unable send message"),
                    PURPLE_MESSAGE_ERROR, time(0));
}

// send back receipt
static void send_receipt(LwqqAsyncEvent* ev, LwqqMsg* msg, char* serv_id,
                         char* what, long retry)
{
   ...
   if (mmsg->upload_retry < 0)
      qq_sys_msg_write(ac, msg->type, serv_id,
                       _("Upload content retry over limit"),
                       PURPLE_MESSAGE_ERROR, time(NULL));
...
// called by purple
static int qq_send_im(PurpleConnection* gc, const gchar* who, const gchar* what,
                      PurpleMessageFlags flags)
{
   ...
   if ((ret = find_group_and_member_by_card(lc, who, &group, &sb))) {
      if (ret == -1 || !sb->group_sig) {
         ...
         lwqq_async_add_event_listener(
             ev, _C_(3pi, qq_send_im, gc, who_, what_, flags));
   ...
   if (send_visual) {
      struct ds whatsnew
          = translate_struct_to_message(ac, mmsg, PURPLE_MESSAGE_SEND);
      PurpleConversation* conv = purple_find_conversation_with_account(
          PURPLE_CONV_TYPE_IM, who, ac->account);
      purple_conversation_write(conv, NULL, ds_c_str(whatsnew), flags,
                                time(NULL));
      ds_free(whatsnew);
   }
...
static int qq_send_chat(PurpleConnection* gc, int id, const char* message,
                        PurpleMessageFlags flags)
{
   qq_account* ac = (qq_account*)purple_connection_get_protocol_data(gc);
...
static GHashTable* qq_chat_info_defaults(PurpleConnection* gc,
                                         const gchar* chat_name)
{
   ...
   if (chat_name != NULL) {
      g_hash_table_insert(defaults, g_strdup(QQ_ROOM_KEY_GID),
                          g_strdup(chat_name));
      g_hash_table_insert(defaults, g_strdup(QQ_ROOM_TYPE),
                          g_strdup(QQ_ROOM_TYPE_QUN));
   }
...
static PurpleRoomlist* qq_get_all_group_list(PurpleConnection* gc)
{
   ...
   field = purple_roomlist_field_new(PURPLE_ROOMLIST_FIELD_STRING, _("QQ"),
                                     QQ_ROOM_KEY_GID, FALSE);
   fields = g_list_append(fields, field);
   field = purple_roomlist_field_new(PURPLE_ROOMLIST_FIELD_STRING, _("Type"),
                                     QQ_ROOM_TYPE, FALSE);
   fields = g_list_append(fields, field);
   ...
   LIST_FOREACH(group, &ac->qq->groups, entries)
   {
      PurpleRoomlistRoom* room = purple_roomlist_room_new(
          PURPLE_ROOMLIST_ROOMTYPE_ROOM, group->name, NULL);
   ...
   {
      PurpleRoomlistRoom* room = purple_roomlist_room_new(
          PURPLE_ROOMLIST_ROOMTYPE_ROOM, discu->name, NULL);
...
static void qq_group_add_or_join(PurpleConnection* gc, GHashTable* data)
{
   ...
   if (type == NULL) {
      ...
      if (group == NULL) {
         ...
         lwqq_async_add_event_listener(
             ev, _C_(2p, search_group_receipt, ev, group));
         return;
      }
   }
   ...
   if (group->data == NULL) {
      ...
      qq_chat_group* cg = group->data;
      PurpleConversation* conv = CGROUP_GET_CONV(cg);
      serv_got_joined_chat(gc, purple_conv_chat_get_id(PURPLE_CONV_CHAT(conv)),
                           try_get(group->account, group->gid));
   }
...
char* qq_get_cb_real_name(PurpleConnection* gc, int id, const char* who)
{
   ...
   if (purple_find_buddy(ac->account, who) != NULL)
      ...
      snprintf(conv_name, sizeof(conv_name), "%s ### %s",
               (sb->card) ? sb->card : sb->nick, group->name);
...
static void qq_change_markname(PurpleConnection* gc, const char* who,
                               const char* alias)
{
   qq_account* ac = purple_connection_get_protocol_data(gc);
   if (ac->disable_send_server)
      return;
   LwqqClient* lc = ac->qq;
   LwqqBuddy* buddy = (ac->flag & QQ_USE_QQNUM)
                          ? find_buddy_by_qqnumber(lc, who)
                          : find_buddy_by_uin(lc, who);
   if (buddy == NULL)
      return;
...
static void change_category_back(LwqqAsyncEvent* event, void* data)
{
   ...
   if (event->result != LWQQ_EC_OK) {
      ...
      purple_notify_error(ac->gc, NULL, _("Change friend category failed"),
                          _("Server fault returns"));
      return;
   }
clean:
   s_free(d[1]);
...
static void add_passerby_to_friend_failed(LwqqAsyncEvent* ev, LwqqBuddy* b,
                                          void* data)
{
   LwqqClient* lc = ev->lc;
   qq_account* ac = lc->data;
   ...
   if (ev->result == 0) {
      purple_notify_message(
          ac->gc, PURPLE_NOTIFY_MSG_INFO, NULL, _("Add passerby as friend"),
          _("Send add request successful"), move_buddy_back, data);
   } else
      purple_notify_message(ac->gc, PURPLE_NOTIFY_MSG_ERROR, NULL,
                            _("Add friend failed"), _("???"), move_buddy_back,
                            data);
}
static void qq_change_category(PurpleConnection* gc, const char* who,
                               const char* old_group, const char* new_group)
{
   ...
   if (strcmp(new_group, QQ_DEFAULT_CATE) == 0)
      ...
      if (cate == NULL) {
         purple_notify_message(gc, PURPLE_NOTIFY_MSG_ERROR, NULL,
                               _("Change friend category failed"),
                               _("no such category exists"), move_buddy_back,
                               data);
         return;
      } else
   ...
   if (buddy->cate_index == LWQQ_FRIEND_CATE_IDX_PASSERBY) {
      ...
      lwqq_async_add_event_listener(
          ev, _C_(3p, add_passerby_to_friend_failed, ev, buddy, data));
      return;
   }
   ...
   lwqq_async_add_event_listener(event,
                                 _C_(2p, change_category_back, event, data));
}
// keep this empty to ensure rename category dont crash
static void qq_rename_category(PurpleConnection* gc, const char* old_name,
                               PurpleGroup* group, GList* moved_buddies)
{
}
static void qq_remove_buddy(PurpleConnection* gc, PurpleBuddy* buddy,
                            PurpleGroup* group)
{
   qq_account* ac = purple_connection_get_protocol_data(gc);
   LwqqClient* lc = ac->qq;
...
static void qq_remove_buddy_oneside_confirm(LwqqConfirmTable* ct,
                                            LwqqClient* lc, LwqqBuddy* bu)
{
   if (ct->answer == LWQQ_YES) {
...
static void qq_remove_buddy_oneside(PurpleBuddy* buddy)
{
   ...
   ct->body = s_strdup(_("This would delete your friends,but keep yourself in "
                         "friend's buddy list\nAre you sure?"));
...
{
   char url[256] = { 0 };
   snprintf(url, sizeof(url), OPEN_PROG " 'http://user.qzone.qq.com/%s'",
            buddy->qqnumber);
   system(url);
}
...
static void qq_send_mail(PurpleBlistNode* n)
{
   ...
   if (find_group_and_member_by_card(lc, purple_buddy_get_name(b), &g, &sb)) {
      ...
      buddy = b->proto_data;
      if (buddy->email)
         snprintf(buf, sizeof(buf), OPEN_PROG " 'mailto: %s<%s>'",
                  buddy->markname ?: buddy->nick, buddy->email);
      else if (buddy->qqnumber)
         snprintf(buf, sizeof(buf), OPEN_PROG " 'mailto: %s<%s@qq.com>'",
                  buddy->markname ?: buddy->nick, buddy->qqnumber);
      else
         return;
   }
   system(buf);
...
static void add_friend_ask_message(LwqqClient* lc, LwqqConfirmTable* ask,
                                   LwqqBuddy* b)
{
                       ...
                       char* message)
{
   ...
   if (message == NULL) {
      ...
      lwqq_async_add_event_listener(ev,
                                    _C_(3p, receipt_notify_error, lc->data,
                                        strdup(_("Add Friend Failed")), ev));
   }
done:
   lwqq_ct_free(c);
   lwqq_buddy_free(b);
                                 ...
                                 char* uni_id, char* message)
{
   ...
   if (err == 10000) {
      ...
      lwqq_async_add_event_listener(
          event, _C_(4p, search_buddy_receipt, event, buddy, uni_id, message));
      return;
   }
   if (err == LWQQ_EC_NO_RESULT) {
...
static void qq_add_buddy(PurpleConnection* pc, PurpleBuddy* buddy,
                         PurpleGroup* group
#if PURPLE_OUTDATE
                         )
#else
...
#endif
{
   ...
   if (find_group_and_member_by_card(lc, uni_id, &g, &sb)) {
      ...
      lwqq_async_evset_add_event(set, ev);
      lwqq_async_add_evset_listener(
          set, _C_(3p, lwqq_info_add_group_member_as_friend, lc, friend, NULL));
      lwqq_async_evset_unref(set);
   } else {
...
#endif
      lwqq_async_add_event_listener(ev,
                                    _C_(4p, search_buddy_receipt, ev, friend,
                                        s_strdup(uni_id), s_strdup(msg)));
   }
   // delete on next cycle
   lwqq_client_dispatch(
       lc, _C_(2p, delete_back_broken_buddy, pc, s_strdup(buddy->name)));
}
//#endif
static gboolean qq_send_attention(PurpleConnection* gc, const char* username,
                                  guint type)
{
   qq_account* ac = gc->proto_data;
   LwqqClient* lc = ac->qq;
...
static char* qq_status_text(PurpleBuddy* pb)
{
   ...
   if (!buddy || !buddy->long_nick)
      return NULL;
   return purple_markup_escape_text(buddy->long_nick, strlen(buddy->long_nick));
}
static void qq_tooltip_text(PurpleBuddy* pb, PurpleNotifyUserInfo* info,
                            gboolean full)
{
   char buf[32];
   LwqqBuddy* buddy = pb->proto_data;
   ...
   purple_notify_user_info_add_pair(info, _("Client"),
                                    qq_client_to_str(buddy->client_type));
}
#if 0
...
#else
      lwqq_async_add_event_listener(lwqq_info_mask_group(lc, g, ct->answer),
...
static void qq_display_self_card(LwqqClient* lc, LwqqBusinessCard* card)
{
   ...
   purple_request_field_group_add_field(g, f);
   f = purple_request_field_string_new("remark", _("Remark"), card->remark,
                                       TRUE);
   purple_request_field_group_add_field(g, f);

   qq_account* ac = lc->data;
   purple_request_fields(ac->gc, _("Group Business Card"), NULL, NULL, fields,
                         _("Update"), G_CALLBACK(self_card_ok), _("Cancel"),
                         G_CALLBACK(self_card_cancel), ac->account, NULL, NULL,
                         _P_(2p, card, lc));
}
...
static void set_group_alias(PurpleBlistNode* node, const char* mark)
{
   ...
   lwqq_async_add_event_listener(
       ev, _C_(2p, set_group_alias_local, node, s_strdup(mark)));
   // LwdbExtension has already done this
   // lwqq_async_add_event_listener(ev,
   // _C_(2p,lwdb_userdb_update_group_info,ac->db, &group));
}
static void qq_set_group_alias(PurpleBlistNode* node)
{
   ...
   if (strcmp(type, QQ_ROOM_TYPE_QUN) == 0)
      purple_request_input(
          ac->gc, _("Modify Mark"), _("Input Mark"), NULL, NULL, FALSE, FALSE,
          NULL, _("Update"), G_CALLBACK(set_group_alias), _("Cancel"),
          G_CALLBACK(do_no_thing), ac->account, NULL, NULL, node);
   else
      purple_request_input(
          ac->gc, _("Set Topic"), _("Input Topic"),
          _("Note:You are setting discussion topic on server.\nWhich would "
            "affect all discussion member"),
          NULL, FALSE, FALSE, NULL, "设置", G_CALLBACK(set_group_alias), "取消",
          G_CALLBACK(do_no_thing), ac->account, NULL, NULL, node);
}
...
#define ADD_STRING(k, v) purple_notify_user_info_add_pair(info, k, v)
   ...
   purple_notify_userinfo(
       ac->gc, g->account, info,
       (PurpleNotifyCloseCallback)purple_notify_user_info_destroy, info);
#undef ADD_STRING
#ifdef QQ_LOCAL_MASK
   qq_chat_group* cg = g->data;
...
static GList* qq_blist_node_menu(PurpleBlistNode* node)
{
   ...
   if (PURPLE_BLIST_NODE_IS_BUDDY(node)) {
      ...
      action = purple_menu_action_new(
          _("Visit QZone"), (PurpleCallback)qq_visit_qzone, node, NULL);
      act = g_list_append(act, action);
      // webqq doesn't support send offline file anymore
      // action = purple_menu_action_new(_("Send Offline
      // File"),(PurpleCallback)qq_send_offline_file,node,NULL);
      // act = g_list_append(act,action);
      action = purple_menu_action_new(_("Send Email"),
                                      (PurpleCallback)qq_send_mail, node, NULL);
      act = g_list_append(act, action);
      // action = purple_menu_action_new(_("Merge Online
      // History"),(PurpleCallback)qq_merge_online_history,buddy,NULL);
      // act = g_list_append(act,action);
      action = purple_menu_action_new(_("Delete Oneside"),
                                      (PurpleCallback)qq_remove_buddy_oneside,
                                      buddy, NULL);
      act = g_list_append(act, action);
      ...
      if (group && group->type == LWQQ_GROUP_QUN) {
         action = purple_menu_action_new(_("Get Information"),
                                         ...
                                         node, NULL);
         act = g_list_append(act, action);
         action = purple_menu_action_new(_("Change Business Card"),
      ...
      act = g_list_append(act, action);
      action = purple_menu_action_new(
          _("Change Mark"), (PurpleCallback)qq_set_group_alias, node, NULL);
      act = g_list_append(act, action);
      action = purple_menu_action_new(
          _("Quit Group"), (PurpleCallback)qq_quit_group, node, NULL);
      act = g_list_append(act, action);
   }
   return act;
}
static void client_connect_signals(PurpleConnection* gc)
{
   ...
   purple_signal_connect(
       purple_conversations_get_handle(), "conversation-created", h,
...
#define ADD_HEADER(s) purple_notify_user_info_add_section_break(info)
   //#define ADD_HEADER(s)     purple_notify_user_info_add_section_header(info,
   //s)
   // ADD_HEADER("基本信息");
   ...
   // ADD_INFO("简介","");
   purple_notify_userinfo(
       gc, who ? who : try_get(b->qqnumber, b->uin), info,
       (PurpleNotifyCloseCallback)purple_notify_user_info_destroy, info);
...
static void qq_get_user_info(PurpleConnection* gc, const char* who)
{
   ...
   if (buddy) {
      ...
      lwqq_async_evset_add_event(set, ev);
      lwqq_async_add_evset_listener(
          set, _C_(3p, display_user_info, gc, buddy, NULL));
      lwqq_async_evset_unref(set);
   } else {
      ...
      if (find_group_and_member_by_card(lc, who, &g, &sb)) {
         ...
         lwqq_async_evset_add_event(set, ev);
         lwqq_async_add_evset_listener(
             set, _C_(3p, display_user_info, gc, buddy, s_strdup(who)));
         lwqq_async_evset_unref(set);
      }
   }
}
static void qq_add_buddies_to_discu(PurpleConnection* gc, int id,
                                    const char* message, const char* local_id)
{
   qq_account* ac = gc->proto_data;
   LwqqClient* lc = ac->qq;
   PurpleConversation* conv = purple_find_chat(gc, id);
   ...
   if (b == NULL) {
      purple_notify_warning(ac->account, _("Warning"),
                            _("Coundn't find friend"), local_id);
      return;
   }
   ...
   lwqq_async_add_event_listener(
       ev, _C_(3p, receipt_notify_error, ac,
               s_strdup(_("Change discu member failed")), ev));
}
...
static void init_plugin(PurplePlugin* plugin)
{
   ...
   option = purple_account_option_bool_new(
       _("Ignore Received Message Font Family"), "disable_custom_font_face",
       FALSE);
   options = g_list_append(options, option);
   option
       = purple_account_option_bool_new(_("Ignore Received Message Font Size"),
                                        "disable_custom_font_size", FALSE);
   options = g_list_append(options, option);
   option = purple_account_option_bool_new(_("Text Lighter In Dark Theme"),
                                           "dark_theme_fix", FALSE);
   options = g_list_append(options, option);
   option = purple_account_option_bool_new(_("SSL(encrypt on chat)"), "ssl",
                                           FALSE);
   options = g_list_append(options, option);
   // disable duplicated message, it seems no use
   // option = purple_account_option_bool_new(_("Remove Duplicated
   // Message"),"remove_duplicated_msg",FALSE);
   // options = g_list_append(options, option);
   option = purple_account_option_bool_new(
       _("Don't Download Group Pic(Reduce Network Transfer)"),
       "no_download_group_pic", FALSE);
   options = g_list_append(options, option);
   option = purple_account_option_bool_new(_("Version Statics"),
                                           "version_statics", TRUE);
   options = g_list_append(options, option);
   option = purple_account_option_bool_new(_("What you seen Is What you send"),
                                           "send_visualbility",
                                           SEND_VISUAL_DEFAULT);
   options = g_list_append(options, option);
#if 0 | DISABLED_AREA
   ...
   option = purple_account_option_string_new(
       _("Ignore Group(Compatible with recent plugin)"), "recent_group_name",
       "Recent Contacts");
   options = g_list_append(options, option);
...
static void version_statics_dlg(qq_account* ac)
{
   ...
   if (v == NULL) {
      ...
      snprintf(body, sizeof(body),
               _("To Support the sustained development of pidgin-lwqq,\n"
                 "Need statistics version count!Really Just a version number "
                 "only!!,following releated information:\n"
                 "Note:https://github.com/xiehuc/pidgin-lwqq/wiki/"
                 "version-statistics\n"
...
static TABLE_BEGIN(proxy_map, long, 0)
    ...
    TR(PURPLE_PROXY_SOCKS5, LWQQ_HTTP_PROXY_SOCKS5) TABLE_END()
...
static void init_client_events(LwqqClient* lc)
{
   ...
   lwqq_add_event(lc->events->poll_msg, _C_(p, qq_msg_check, lc));
   lwqq_add_event(lc->events->poll_lost, _C_(p, lost_connection, lc));
   ...
   lwqq_add_event(lc->events->upload_fail,
                  _C_(4p, upload_content_fail, lc, &lc->args->serv_id,
...
static void start_login(PurpleAccount* account)
{
   ...
   if (password == NULL || strcmp(password, "") == 0) {
      purple_connection_error_reason(
          pc, PURPLE_CONNECTION_ERROR_AUTHENTICATION_FAILED,
          _("Password is empty"));
      return;
   }
   if (strcmp(lwqq_version, VERSION) != 0) {
      ...
      snprintf(buf, sizeof(buf),
               _("lwqq version didn't match, found %s, require %s"),
               lwqq_version, VERSION);
   ...
   if (ac->db) {
      ...
      lwqq_override(ac->font.family,
                    s_strdup(lwdb_userdb_read(ac->db, "f_family")));
      ac->font.size = s_atoi(lwdb_userdb_read(ac->db, "f_size"), ac->font.size);
      ac->font.style
          = s_atoi(lwdb_userdb_read(ac->db, "f_style"), ac->font.style);

      const char* last_hash = lwdb_userdb_read(ac->db, "last_hash");
      if (last_hash)
         lwqq_hash_set_beg(ac->qq, last_hash);
   }
   ...
   lwqq_http_proxy_set(lwqq_get_http_handle(ac->qq), proxy_map(proxy->type),
